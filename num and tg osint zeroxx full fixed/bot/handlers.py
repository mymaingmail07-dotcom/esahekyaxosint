...
_VEHICLE_NUMBER_PATTERN = re.compile(r"^[A-Z]{2}[0-9]{1,2}(?:[A-Z]{1,3})?[0-9]{1,4}$")
...
def _normalize_vehicle_number(value: str) -> str | None:
...
def _inject_developer_credit(payload: Any) -> Any:
    if not isinstance(payload, dict):
        return payload
    updated = dict(payload)
    updated["Developer"] = "@brutoixx"
    return updated
...
async def _send_vehicle_api_response(message: Message, payload: Any, is_json: bool, lookup_value: str) -> None:
    response_text = _pretty_api_response(payload, is_json)
    if not response_text:
        await message.answer(messages.NO_METADATA)
        return

    rendered = f"<pre>{html.escape(response_text)}</pre>\n{messages.VEHICLE_RESPONSE_DELETE_NOTICE}"
    if len(rendered) > _TELEGRAM_MESSAGE_LIMIT:
        await _send_txt_response(message, response_text, lookup_value)
        return
    try:
        ...
        if "message is too long" in str(exc).lower() or len(rendered) > _TELEGRAM_MESSAGE_LIMIT:
            ...
            logger.warning("Telegram rejected vehicle API response delivery: %s", exc)
            await message.answer(messages.API_RESPONSE_DELIVERY_ERROR)
    except (TelegramForbiddenError, TelegramAPIError):
        logger.warning("Telegram could not deliver vehicle API response")
        await message.answer(messages.API_RESPONSE_DELIVERY_ERROR)
    except Exception:
        logger.exception("Unexpected vehicle API response delivery failure")
        await message.answer(messages.API_RESPONSE_DELIVERY_ERROR)
...
async def commands_handler(message: Message, settings: Settings) -> None:
    normal = ["/start", "/help", "/num", "/vnum", "/tg", "/cmds"]
...
@router.message(Command("vnum"))
async def vehicle_number_handler(message: Message, api_client: NumberApiClient, settings: Settings) -> None:
    text = message.text or ""
    parts = text.split(maxsplit=1)
    candidate = parts[1].strip() if len(parts) == 2 else ""
    if not candidate:
        await message.answer(messages.INVALID_VEHICLE_NUMBER)
        return
    vehicle_number = _normalize_vehicle_number(candidate)
    if not vehicle_number:
        await message.answer(messages.INVALID_VEHICLE_FORMAT)
        return
    user_id = message.from_user.id
    now = time.monotonic()
    if now - _last_lookup.get(user_id, 0) < settings.num_rate_limit_seconds:
    ...
    _last_lookup[user_id] = now
    try:
        response = await api_client.lookup_vehicle(vehicle_number)
        payload = _inject_developer_credit(response.payload) if response.is_json and isinstance(response.payload, dict) else response.payload
        _audit_api_response.set(_api_result_text(payload))
        await _send_vehicle_api_response(message, payload, response.is_json, vehicle_number)
    except NumberApiTimeoutError:
        await message.answer(messages.LOOKUP_TIMEOUT)
    except NumberApiConnectionError:
        await message.answer(messages.LOOKUP_CONNECTION_ERROR)
